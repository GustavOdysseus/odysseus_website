...
class QuestionAnalyzer:
    ...
    def analyze_question(self, question: Dict[str, str]) -> Dict:
        """Analisa uma questão usando GPT-4-mini com análise detalhada."""
...
'''
Matéria Principal: [matéria específica, ex: Língua Portuguesa - Análise Sintática]

Tópico Específico: [tópico exato com a regra gramatical aplicada, ex: Predicação Verbal - Identificação e uso de Verbos Transitivos Diretos]

Subtópicos Relacionados:
1. [subtópico específico com regra aplicada, ex: Identificação do complemento verbal direto sem preposição]
2. [outro subtópico específico com exemplo]
3. [outro subtópico específico com exemplo]

Nível de Dificuldade: [1-5]
Justificativa da Dificuldade: [explicação detalhada do porquê deste nível]

Habilidades Avaliadas:
1. [habilidade específica com exemplo prático]
2. [habilidade específica com exemplo prático]
3. [habilidade específica com exemplo prático]

Armadilhas da Questão:
1. [armadilha específica com exemplo]
2. [armadilha específica com exemplo]

Temas Correlatos para Estudo:
1. [tema específico com referência à regra gramatical]
2. [tema específico com referência à regra gramatical]
3. [tema específico com referência à regra gramatical]
'''
"""
        ...
        try:
            ...
            materia = re.search(r'Matéria Principal:\s*(.+)', docstring).group(1).strip()
            topico = re.search(r'Tópico Específico:\s*(.+)', docstring).group(1).strip()
            subtopicos = re.findall(r'\d\.\s*(.+)', docstring.split('Subtópicos Relacionados:')[1].split('Nível')[0])
            ...
            habilidades = re.findall(r'\d\.\s*(.+)', docstring.split('Habilidades Avaliadas:')[1].split('Armadilhas')[0])
            armadilhas = re.findall(r'\d\.\s*(.+)', docstring.split('Armadilhas da Questão:')[1].split('Temas')[0])
            temas = re.findall(r'\d\.\s*(.+)', docstring.split('Temas Correlatos para Estudo:')[1])
            
            result = {
                'id': question['id'],
                'texto_questao': question['content'],
                'materia_principal': materia,
                'topico_especifico': topico,
    ...
    def generate_visualizations(self, results: List[Dict], output_dir: str = 'resultados'):
        ...
        for _, row in df.iterrows():
            topico = row['topico_especifico']
            if topico not in analise_detalhada['topicos']:
                analise_detalhada['topicos'][topico] = {
                    'count': 0,
                    'dificuldade_media': 0,
                    'questoes_relacionadas': [],
                    'armadilhas_comuns': set(),
                    'temas_estudo': set()
                }
            
            # Atualizar contadores e análises
            info = analise_detalhada['topicos'][topico]
            info['count'] += 1
            info['dificuldade_media'] += row['dificuldade']
            info['questoes_relacionadas'].append(row['id'])
            info['armadilhas_comuns'].update(row['armadilhas'])
            info['temas_estudo'].update(row['temas_estudo'])
        
        # Gerar relatório detalhado
        with open(os.path.join(output_dir, 'analise_detalhada.txt'), 'w', encoding='utf-8') as f:
            f.write("ANÁLISE DETALHADA DAS QUESTÕES\n\n")
            
            for topico, dados in sorted(analise_detalhada['topicos'].items(), 
                                      key=lambda x: x[1]['count'], reverse=True):
                f.write(f"\n{'='*50}\n")
                f.write(f"TÓPICO: {topico}\n")
                f.write(f"{'='*50}\n")
                f.write(f"Quantidade de questões: {dados['count']}\n")
                f.write(f"Dificuldade média: {dados['dificuldade_media']/dados['count']:.2f}/5\n")
                
                f.write("\nARMADILHAS MAIS COMUNS:\n")
                for armadilha in dados['armadilhas_comuns']:
                    f.write(f"• {armadilha}\n")
                
                f.write("\nTEMAS PARA ESTUDO APROFUNDADO:\n")
                for tema in dados['temas_estudo']:
                    f.write(f"• {tema}\n")
                
                f.write("\nQUESTÕES RELACIONADAS:\n")
                f.write(f"IDs: {', '.join(map(str, dados['questoes_relacionadas']))}\n")
                
                f.write(f"\n{'-'*50}\n")
        
        # Criar visualizações tradicionais
        self._create_standard_visualizations(df, output_dir)

    def _create_standard_visualizations(self, df, output_dir):
        # 1. Distribuição de Matérias
        fig_materias = px.pie(df, names='materia_principal', title='Distribuição de Matérias')
        fig_materias.write_html(os.path.join(output_dir, 'distribuicao_materias.html'))
        
        # 2. Nível de Dificuldade por Matéria
        fig_dificuldade = px.box(df, x='materia_principal', y='dificuldade', 
