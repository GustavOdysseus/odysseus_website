...
class Crew(BaseModel):
    """
    Representa um grupo de agentes, definindo como eles devem colaborar e as tarefas que devem realizar.

    Atributos:
        tasks: Lista de tarefas atribuídas à equipe.
        agents: Lista de agentes que fazem parte desta equipe.
        manager_llm: O modelo de linguagem que irá executar o agente gerente.
        manager_agent: Agente personalizado que será usado como gerente.
        memory: Indica se a equipe deve usar memória para armazenar as memórias de sua execução.
        memory_config: Configuração para a memória a ser usada pela equipe.
        cache: Indica se a equipe deve usar um cache para armazenar os resultados da execução das ferramentas.
        function_calling_llm: O modelo de linguagem que irá executar a chamada de ferramentas para todos os agentes.
        process: O fluxo de processo que a equipe seguirá (por exemplo, sequencial, hierárquico).
        verbose: Indica o nível de verbosidade para log durante a execução.
        config: Configurações de configuração para a equipe.
        max_rpm: Número máximo de requisições por minuto que a execução da equipe deve respeitar.
        prompt_file: Caminho para o arquivo json de prompt a ser usado pela equipe.
        id: Um identificador único para a instância da equipe.
        task_callback: Callback a ser executado após cada tarefa para a execução de cada agente.
        step_callback: Callback a ser executado após cada etapa para a execução de cada agente.
        share_crew: Indica se você deseja compartilhar as informações completas da equipe e a execução com a crewAI para melhorar a biblioteca e permitir que treinem modelos.
        planning: Planejar a execução da equipe e adicionar o plano à equipe.
    """
